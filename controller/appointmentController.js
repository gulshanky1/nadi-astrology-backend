...
export const verifyAppointmentPayment = async (req, res) => {
  ...
  try {
    ...
    const providerMail = {
      from: process.env.SMTP_USER,
      to: process.env.CONTACT_RECEIVER_EMAIL,
      subject: `🆕 New Appointment - ${appointmentData?.name || "Unknown"}`,
      html: `
    ...
    const userMail = {
      ...
      html: `
        ...
        <p>We will contact you ${
          appointmentData?.consultationType === "urgent"
            ? "within 2 to 4 hours (urgent consultation)."
            : "within 2 working days (excluding weekends)."
        }</p>
        <br/>
        <p>Warm regards,<br/>Nadi Astrology Team</p>
      `,
    };
    ...
    return res.status(200).json({
      message: "Appointment payment verified and emails sent.",
    });
  } catch (err) {
    console.error("❌ Email sending failed:", err);
    return res.status(500).json({
      error: "Payment verified but email sending failed.",
    });
  }
};
